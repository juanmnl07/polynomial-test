...
class LinearRegression
{
    ...
    public function Calc()
    {
        ...
        for($k=3; $k < 15 ;$k++) {
            if($this->ia != 0) {
                if(abs($this->ia) < 1 && abs($this->ia) >= 0 && number_format($this->ia,$k-2) != 0) {
                    $this->ia = doubleval(number_format($this->ia,$k));
                    break;
                }
            }
        }
        ...
        for($k=3; $k < 15 ;$k++) {
            if($this->ib != 0) {
                if(abs($this->ia) < 1 && abs($this->ia) >= 0 && number_format($this->ib,$k-2) != 0) {
                    $this->ib = doubleval(number_format($this->ib,$k));
                    break;
                }
            }
        }
    ...
    public function GetY($aMinX, $aMaxX, $aStep = 1)
    {
        ...
        for ($x = $aMinX; $x <= $aMaxX; $x += $aStep) {
            $xx[] = $x;
            $yy[] = ($this->interpolate($x) > 0) ? $this->interpolate($x) : null;
        }
    ...
    public function interpolate($x) {
        return $this->ia + $this->ib * $x; // y=a+bx
    }
...
class ExponentialRegression
{
    ...
    public function Calc()
    {
        ...
        for ($i = 0; $i < $n; ++$i) {
            if($this->iy[$i] != 0) {
                $sx2y += pow($this->ix[$i], 2) * $this->iy[$i];
                $sylny += $this->iy[$i] * log($this->iy[$i]);
                $sxy += $this->ix[$i] * $this->iy[$i];
                $sxylny += $this->ix[$i] * $this->iy[$i] * log($this->iy[$i]);
                $sy += $this->iy[$i];
            }
        }

        if ($sy * $sx2y - pow($sxy, 2) > __LR_EPSILON) {
            $this->ia = exp(($sx2y * $sylny - $sxy * $sxylny) / ($sy * $sx2y - pow($sxy, 2)));
            $this->ib = ($sy * $sxylny - $sxy * $sylny) / ($sy * $sx2y - pow($sxy, 2));
        } else {
            $this->ia = 0;
            $this->ib = 0;
        }
    ...
    public function interpolate($x) {
        return $this->ia * exp($this->ib * $x); // y=ae^(bx)
    }
...
class LogarithmicRegression
{
    ...
    public function Calc()
    {
        ...
        for ($i = 0; $i < $n; ++$i) {
            if($this->ix[$i] != 0) {
                $sylnx += $this->iy[$i] * log($this->ix[$i]);
                $sy += $this->iy[$i];
                $slnx += log($this->ix[$i]);
                $slnx2 += pow(log($this->ix[$i]), 2);
            }
        }

        if (/*$slnx2 - pow($slnx,2) > __LR_EPSILON && $n > __LR_EPSILON*/ true) {
            $this->ib = (($n * $sylnx - $sy * $slnx) / ($n * $slnx2 - pow($slnx, 2)));
            $this->ia = (($sy - $this->ib * $slnx) / ($n));
        } else {
            $this->ib = 0;
            $this->ia = 0;
        }
    ...
    public function interpolate($x) {
        return $this->ia + $this->ib * log($x); // y=a+b*ln(x)
    }
...
class PowerRegression
{
    ...
    public function Calc()
    {
        ...
        for ($i = 0; $i < $n; ++$i) {
            if($this->ix[$i] != 0  && $this->iy != 0) {
                $slnxlny += log($this->ix[$i]) * log($this->iy[$i]);
                $slnx += log($this->ix[$i]);
                $slny += log($this->iy[$i]);
                $slnx2 += pow(log($this->ix[$i]), 2);
            }
        ...
        if (/*$slnx2 - pow($slnx,2) > __LR_EPSILON && $n > __LR_EPSILON*/
        true
        ) {
            $this->ib = (($n * $slnxlny - $slnx * $slny) / ($n * $slnx2 - pow($slnx, 2)));
            $this->ia = (exp(($slny - $this->ib * $slnx) / ($n)));
        } else {
            $this->ib = 0;
            $this->ia = 0;
        }
    ...
    public function interpolate($x) {
        return $this->ia * pow($x, $this->ib); // y=a*x^(b)
    }
...
class PolynomialRegression
{
    ...
    {
        $this->numberOfCoefficient = $numberOfCoefficient + 1;
        $this->reset();
    ...
    public function getCoefficients($numberOfCoefficient = -1)
    {
        ...
        foreach($coefficients as $key=>$v) {
            for($k=3; $k < 50 ;$k++) {
                if($v != 0) {
                    if(abs($v) < 1 && abs($v) >= 0 && number_format($v,$k-4) != 0) {
                        $v = doubleval(number_format($v,$k));
                        $coefficients[$key] = $v;
                        break;
                    }
                    else {
    ...
    public function interpolate($x)
    {
        ...
        for ($coefficentIndex = 0; $coefficentIndex < $numberOfCoefficient; ++$coefficentIndex) {
            $y = $y + $coefficients[$coefficentIndex] * pow($x, $coefficentIndex);
        }
    ...
    public function GetEquation()
    {
        ...
        for ($coefficentIndex = 0; $coefficentIndex < $numberOfCoefficient; ++$coefficentIndex) {
            if ($equation == "") {
                if($coefficients[$coefficentIndex] >= 0) {
                    $equation .= $coefficients[$coefficentIndex];
                }
                else {
                    $equation .= " - " . (substr(strval($coefficients[$coefficentIndex]),1));
                }
            } else if ($coefficients[$coefficentIndex] != 0) {
                if($coefficients[$coefficentIndex] >= 0) {
                    if($coefficentIndex > 1) {
                        $equation .= " + " . $coefficients[$coefficentIndex] . " * X^" . $coefficentIndex;
                    }
                    else {
                        $equation .= " + " . $coefficients[$coefficentIndex] . " * X";
                    }
                }
                else {
                    if($coefficentIndex > 1) {
                        $equation .= " - " . (substr(strval($coefficients[$coefficentIndex]),1)) . " * X^" . $coefficentIndex;
                    }
                    else {
                        $equation .= " - " . (substr(strval($coefficients[$coefficentIndex]),1)) . " * X";
                    }
                }
            }
        }
